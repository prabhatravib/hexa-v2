...
function App() {
  ...
  useEffect(() => {
    const loadExternalContent = async () => {
      ...
      // Load external data using session ID
      const loadExternalData = async () => {
        try {
          // Import sessionIsolation inline to avoid issues with module loading
          const { sessionIsolation } = await import('@/lib/sessionIsolation');

          // Get CLIENT session ID (from URL param or cached standalone ID)
          const clientSessionId = sessionIsolation.getCurrentSessionId();

          console.log('ðŸ†" Using client session ID for external data:', clientSessionId);

          if (clientSessionId) {
            // Check for external data using the client session ID
            const statusResponse = await fetch(`/api/external-data/status?sessionId=${clientSessionId}`);
